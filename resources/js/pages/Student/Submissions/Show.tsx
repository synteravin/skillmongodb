...
<div className="scrollbar-thin scrollbar-track-transparent scrollbar-thumb-blue-200 flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto rounded-xl border border-blue-200 bg-white p-4 shadow-sm sm:gap-5 sm:p-5 md:gap-6 md:p-8 dark:border-blue-500/30 dark:bg-gradient-to-b dark:from-[#0d1229] dark:to-[#080d1e] dark:shadow-none dark:scrollbar-thumb-blue-500/30">
    {/* Header Info */}
    <div className="border-b border-blue-100 pb-4 sm:pb-5 md:pb-6 dark:border-blue-500/15">
        <div className="mb-2 sm:mb-3 flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-md border border-blue-300 bg-blue-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-600 sm:px-3 sm:text-xs dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-400">
        ...
        <p className="mt-2 whitespace-pre-line font-['Oxanium'] text-xs leading-relaxed text-slate-700 sm:mt-3 sm:text-sm md:text-base dark:text-gray-300">
    ...
    <div className="flex flex-col gap-4 sm:gap-5 md:gap-6">
        <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <div>
                ...
                <p className="font-['Oxanium'] text-[10px] font-semibold uppercase tracking-wider text-slate-400 sm:text-xs">
                    Secure Workspace
                </p>
            </div>
        </div>
                ...
                <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <div>
                        ...
                        <p className="font-['Oxanium'] text-[11px] text-emerald-700 sm:text-xs dark:text-emerald-400/80">
                            Your work has been uploaded and is awaiting mentor
                            review.
                        </p>
                    </div>
                </div>
                ...
                <div className="mt-1 flex flex-col gap-2.5 sm:mt-2 sm:gap-3">
                    {studentSubmission?.link && (
                        <div className="rounded-lg border border-emerald-200 bg-white p-3 sm:p-4 dark:border-emerald-500/20 dark:bg-black/20">
                            <label className="mb-1 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-slate-400 sm:text-[10px]">
                                <LinkIcon className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Project URL
                            </label>
                            <a
                                href={studentSubmission.link}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="break-all font-['Oxanium'] text-xs font-medium text-blue-600 underline sm:text-sm dark:text-blue-400"
                            >
                                {studentSubmission.link}
                            </a>
                        </div>
                    ...
                    {studentSubmission?.file_path && (
                        <div className="flex items-center justify-between rounded-lg border border-emerald-200 bg-white p-3 sm:p-4 dark:border-emerald-500/20 dark:bg-black/20">
                            <div>
                                <label className="mb-1 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-slate-400 sm:text-[10px]">
                                    <FileText className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Attached
                                    Document
                                </label>
                    ...
                    {studentSubmission?.notes && (
                        <div className="rounded-lg border border-emerald-200 bg-white p-3 sm:p-4 dark:border-emerald-500/20 dark:bg-black/20">
                            <label className="mb-1 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-slate-400 sm:text-[10px]">
                                <MessageSquare className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Private
                                Notes
                            </label>
                            <div className="font-['Oxanium'] text-xs italic text-slate-600 sm:text-sm dark:text-gray-300">
                                "{studentSubmission.notes}"
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mt-1 flex items-start gap-2.5 rounded-lg border border-amber-300 bg-amber-50 p-3 text-[11px] text-amber-800 sm:mt-2 sm:gap-3 sm:p-3.5 sm:text-xs dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                        ...
                        <div>
                            ...
                            <p className="mt-0.5 font-['Oxanium']">
                                You can overwrite your submission using the form
                                below as long as it hasn't been graded.
                            </p>
                        </div>
                    </div>
                )}
        ...
        {(!isSubmitted || !isGraded) && (
            <form onSubmit={submit} className="flex flex-col gap-4 sm:gap-5 md:gap-6">
                {submission.submission_type === 'link' ? (
                    <div>
                        <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-slate-500 sm:mb-2 sm:text-xs dark:text-gray-400">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-blue-200 bg-white py-3 pl-9 pr-3 text-xs font-medium text-slate-800 outline-none transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 sm:py-3.5 sm:pl-11 sm:pr-4 sm:text-sm dark:border-blue-500/30 dark:bg-white/5 dark:text-white dark:focus:border-blue-400"
                            />
                        </div>
                        {errors.link && (
                            ...
                            {data.file ? (
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="mt-1 text-[11px] text-slate-400 sm:text-xs">
                                        Supported: PDF, ZIP, DOCX (Max 10MB)
                                    </p>
                                </div>
                            )}
                        </div>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        className="w-full rounded-xl border border-blue-200 bg-white p-3 text-xs font-medium text-slate-800 outline-none transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 sm:p-4 sm:text-sm dark:border-blue-500/30 dark:bg-white/5 dark:text-white dark:focus:border-blue-400"
                    />
                    {errors.notes && (
                        <p className="mt-1.5 text-[11px] font-medium text-red-500 sm:text-xs">
                            {errors.notes}
                ...
                <button
                    type="submit"
                    disabled={processing}
                    className="flex items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-5 py-3 font-['Orbitron'] text-[11px] font-bold uppercase tracking-wider text-white shadow-md shadow-blue-600/20 transition-all hover:bg-blue-700 disabled:opacity-50 sm:gap-2 sm:px-6 sm:py-3.5 sm:text-xs"
                >
