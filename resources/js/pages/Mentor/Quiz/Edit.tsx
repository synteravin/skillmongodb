...
export default function Edit({ quiz }: { quiz: Quiz }) {
    ...
    return (
        <AppLayout>
            <div
                className="min-h-screen bg-slate-50 p-4 text-slate-800 sm:p-6 lg:p-8 dark:bg-gradient-to-br dark:from-[#020617] dark:via-[#020617] dark:to-black dark:text-white"
                style={{ fontFamily: "'Outfit', sans-serif" }}
            >
                <div className="mx-auto w-full space-y-6 sm:space-y-8">
                    {/* HEADER */}
                    <div className="border-slate-350 dark:border-slate-850 flex flex-col justify-center gap-4 rounded-2xl border bg-white p-6 shadow-sm backdrop-blur-xl dark:bg-slate-900/50 dark:shadow-lg">
                        <button
                                    ...
                                    className={`rounded-md border px-2.5 py-1 text-xs font-bold tracking-wider uppercase ${
                                        quiz.difficulty === 'hard'
                                            ...
                                            : quiz.difficulty === 'medium'
                                              ...
                                              : 'bg-emerald-550/5 border-emerald-200 text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-400'
                                    }`}
                                >
                    ...
                    <div className="space-y-6">
                        {questions.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-white p-12 text-center shadow-sm dark:border-slate-800/60 dark:bg-slate-900/20">
                                <HelpCircle
                                    size={48}
                                    className="mb-4 text-slate-400 opacity-70 dark:text-slate-600"
                                />
                                <h3 className="text-slate-850 mb-1 text-lg font-semibold dark:text-slate-300">
                                    No questions
                                </h3>
                                ...
                                <button
                                    onClick={addQuestion}
                                    className="flex cursor-pointer items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-indigo-500/20 transition-all hover:bg-indigo-700"
                                >
                                    <Plus size={16} />
                                    Add Question
                                </button>
                    ...
                    {/* ACTIONS */}
                    {questions.length > 0 && (
                        <div className="flex flex-col items-center justify-between gap-4 rounded-2xl border border-slate-300 bg-white p-6 shadow-sm backdrop-blur-xl sm:flex-row dark:border-slate-800/60 dark:bg-slate-900/50 dark:shadow-lg">
                            <button
                                onClick={addQuestion}
                                className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-300 bg-slate-100 px-5 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-200 hover:text-slate-900 sm:w-auto dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white"
                            >
                                <Plus size={16} />
                            ...
                            <button
                                onClick={submit}
                                disabled={loading}
                                className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-indigo-600 px-8 py-2.5 text-sm font-medium text-white shadow-md shadow-indigo-500/10 transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto dark:shadow-indigo-500/20"
                            >
                                {loading ? (
                                    <>
                                        <svg
...
function QuestionCard({
    ...
    return (
        <div className="overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm backdrop-blur-md transition-all duration-300 hover:border-slate-400 dark:border-slate-800/80 dark:bg-slate-900/40 dark:shadow-xl dark:hover:border-slate-700/80">
            {/* HEADER */}
            <div className="flex items-center justify-between border-b border-slate-300 bg-slate-50 p-4 dark:border-slate-800/60 dark:bg-slate-950/50">
                ...
                <button
                    onClick={onDelete}
                    className="cursor-pointer rounded-lg p-2 text-slate-400 transition-colors hover:bg-rose-100 hover:text-rose-600 dark:text-slate-500 dark:hover:bg-rose-500/10 dark:hover:text-rose-400"
            ...
            <div className="space-y-6 p-5 sm:p-6">
                ...
                {/* IMAGE UPLOAD */}
                <div>
                    <label className="text-slate-555 mb-1.5 ml-1 block text-xs font-semibold dark:text-slate-400">
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        {data.media_file && (
                            <span className="text-slate-555 max-w-[200px] truncate py-2.5 text-sm sm:max-w-xs dark:text-slate-400">
                ...
                {/* ANSWERS */}
                <div className="border-t border-slate-300 pt-4 dark:border-slate-800/60">
                        ...
                        {data.answers.map((a, i) => (
                            <div
                                ...
                                className={`flex items-stretch gap-2 rounded-xl border p-2 transition-all duration-200 sm:gap-3 ${
                                    a.is_correct
                                        ? 'border-emerald-300 bg-emerald-50 shadow-sm shadow-emerald-500/5 dark:border-emerald-500/40 dark:bg-emerald-950/20'
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className={`flex items-center justify-center rounded-lg px-3 transition-colors sm:px-4 ${
                                        a.is_correct
                                            ...
                                            : 'bg-slate-200/60 text-slate-500 hover:bg-slate-300/80 dark:bg-slate-800 dark:hover:bg-slate-700'
                                    }`}
                                ...
                                <input
                                    ...
                                    className="flex-1 border-none bg-transparent px-2 py-2.5 text-sm text-slate-800 outline-none placeholder:text-slate-400 focus:ring-0 dark:text-white dark:placeholder:text-slate-600"
                                />

                                {/* DELETE */}
                                <button
                                    onClick={() => removeAnswer(i)}
                                    className="hover:text-rose-650 hover:bg-rose-105 cursor-pointer rounded-lg px-3 text-slate-500 transition-colors hover:bg-rose-100 dark:hover:bg-rose-500/10 dark:hover:text-rose-400"
                                    title="Remove answer"
                                >
                                    <X size={16} />
                                </button>
                    ...
                    <div className="mt-4">
                        <button
                            onClick={addAnswer}
                            className="hover:text-indigo-755 flex cursor-pointer items-center gap-2 rounded-lg bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-600 transition-colors hover:bg-indigo-100 hover:text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400 dark:hover:text-indigo-300"
                        >
                            <Plus size={14} />
                            Add Option
                        </button>
