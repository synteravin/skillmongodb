...
export default function Index({ quizzes }: { quizzes: Quiz[] }) {
    ...
    const handleDeleteQuiz = (quiz: Quiz) => {
        ...
        setConfirmModal({
            ...
            variant: 'danger',
            onConfirm: () => {
                router.delete(`/mentor/quiz/${quiz.id}`);
            },
        });
    };
        ...
        <AppLayout>
            <div className="relative min-h-screen bg-slate-50 dark:bg-[#030712] text-slate-800 dark:text-white p-4 sm:p-6 lg:p-8 w-full mx-auto overflow-hidden">
                ...
                <div className="relative z-10 w-full mx-auto space-y-6 sm:space-y-8">
                    ...
                    {/* HEADER */}
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#060B1A]/80 p-6 rounded-xl border border-slate-300 dark:border-white/8 backdrop-blur-sm shadow-sm dark:shadow-md">
                        <div>
                            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                                <div className="p-2 bg-indigo-50 border border-indigo-200 dark:bg-[#7C5CFF]/15 dark:border-[#7C5CFF]/25 rounded-lg text-indigo-600 dark:text-[#7C5CFF]">
                                    <HelpCircle size={24} />
                                </div>
                                Quiz Management
                            </h1>
                    ...
                    {quizzes.length === 0 ? (
                        <div className="flex flex-col items-center justify-center p-12 border border-dashed border-slate-350 dark:border-white/8 rounded-xl bg-white dark:bg-[#060B1A]/40 text-center shadow-sm">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 text-sm">Create a quiz from the Module Builder to see it here.</p>
                        </div>
                    ) : (
                            ...
                            {quizzes.map((quiz) => (
                                ...
                                >
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="p-2.5 bg-indigo-50 border border-indigo-200 dark:bg-[#7C5CFF]/10 dark:border-[#7C5CFF]/20 rounded-lg text-indigo-600 dark:text-[#7C5CFF] group-hover:scale-105 transition-transform duration-200">
                                            <Layers size={20} />
                                        </div>
                                        <span className={`text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider border ${
                                            quiz.difficulty === 'hard' ? 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-500/10 dark:text-rose-455 dark:border-rose-500/20' :
                                            quiz.difficulty === 'medium' ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-455 dark:border-amber-500/20' :
                                            'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-455 dark:border-emerald-500/20'
                                        }`}>
                                            {quiz.difficulty}
                                        </span>
                                    </div>
                                    ...
                                    <div className="mt-auto flex items-center gap-2 pt-4 border-t border-slate-200 dark:border-white/5">
                                        <button
                                            type="button"
                                            onClick={() => router.get(`/mentor/quiz/${quiz.id}/edit`)}
                                            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-slate-100 hover:bg-[#7C5CFF]/10 dark:hover:bg-[#7C5CFF]/15 hover:text-[#7C5CFF] text-slate-700 dark:text-white rounded-lg text-sm font-semibold transition-colors border border-slate-300 dark:border-white/8 hover:border-[#7C5CFF]/30 cursor-pointer"
                                        >
                                            <Edit2 size={14} />
                                            Edit
                                        </button>
                                        <button
                                            ...
                                            className="px-3 py-2 bg-slate-100 hover:bg-rose-100 dark:hover:bg-rose-500/10 text-slate-600 dark:text-slate-350 hover:text-rose-700 dark:hover:text-rose-455 rounded-lg text-sm font-medium transition-colors border border-slate-300 dark:border-white/8 hover:border-rose-500/30 cursor-pointer"
                                            title="Delete Quiz"
                                        >
                                            <Trash2 size={16} />
                                        </button>
