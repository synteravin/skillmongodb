        ...
        <AppLayout>
            <div className="relative mx-auto min-h-screen w-full overflow-hidden bg-slate-50 p-4 text-slate-800 sm:p-6 lg:p-8 dark:bg-[#030712] dark:text-white">
                ...
                <div className="relative z-10 mx-auto w-full space-y-6 sm:space-y-8">
                    {/* HEADER */}
                    <div className="flex flex-col items-start justify-between gap-4 rounded-xl border border-slate-300 bg-white p-6 shadow-sm backdrop-blur-sm sm:flex-row sm:items-center dark:border-white/8 dark:bg-[#060B1A]/80 dark:shadow-md">
                        <div>
                            <h1 className="flex items-center gap-3 text-2xl font-bold text-slate-900 sm:text-3xl dark:text-white">
                                <div className="rounded-lg border border-indigo-200 bg-indigo-50 p-2 text-indigo-600 dark:border-[#7C5CFF]/25 dark:bg-[#7C5CFF]/15 dark:text-[#7C5CFF]">
                                    <HelpCircle size={24} />
                                </div>
                                Quiz Management
                            </h1>
                            ...
                            {quizzes.map((quiz) => (
                                ...
                                >
                                    <div className="mb-4 flex items-start justify-between">
                                        <div className="rounded-lg border border-indigo-200 bg-indigo-50 p-2.5 text-indigo-600 transition-transform duration-200 group-hover:scale-105 dark:border-[#7C5CFF]/20 dark:bg-[#7C5CFF]/10 dark:text-[#7C5CFF]">
                                            <Layers size={20} />
                                        </div>
                                        <span
                                            className={`rounded-lg border px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase sm:text-xs ${
                                                quiz.difficulty === 'hard'
                                                    ? 'dark:text-rose-455 border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-500/20 dark:bg-rose-500/10'
                                                    : quiz.difficulty ===
                                                        'medium'
                                                      ? 'dark:text-amber-455 border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-500/20 dark:bg-amber-500/10'
                                                      : 'dark:text-emerald-455 border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10'
                                            }`}
                                        >
                                            {quiz.difficulty}
                                        </span>
                                    ...
                                    <div className="mt-auto flex items-center gap-2 border-t border-slate-200 pt-4 dark:border-white/5">
                                        <button
                                            ...
                                            onClick={() =>
                                                router.get(
                                                    `/mentor/quiz/${quiz.id}/edit`,
                                                )
                                            }
                                            className="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-lg border border-slate-300 bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:border-[#7C5CFF]/30 hover:bg-[#7C5CFF]/10 hover:text-[#7C5CFF] dark:border-white/8 dark:text-white dark:hover:bg-[#7C5CFF]/15"
                                        >
                                            <Edit2 size={14} />
                                            Edit
                                        </button>
                                        <button
                                            ...
                                            className="dark:text-slate-350 dark:hover:text-rose-455 cursor-pointer rounded-lg border border-slate-300 bg-slate-100 px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:border-rose-500/30 hover:bg-rose-100 hover:text-rose-700 dark:border-white/8 dark:hover:bg-rose-500/10"
                                            title="Delete Quiz"
                                        >
                                            <Trash2 size={16} />
                                        </button>
