...
export default function SettingsLayout({ children }: PropsWithChildren) {
    ...
    return (
        <div className={cn("px-4 py-6", isAdminOrMentor && "px-6 py-8 sm:px-8 lg:px-10")}>
            {isAdminOrMentor ? (
                <div className="relative overflow-hidden rounded-xl border border-slate-200/80 bg-[#f5f6ff] p-6 shadow-sm sm:p-8 md:p-10 dark:border-slate-800 dark:bg-[#0d0f17] mb-8">
                    {/* Grid Pattern Motif */}
                    <div
                    ...
                    <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                        <div className="space-y-3">
                            ...
                            <h1 className="text-2xl leading-snug font-semibold tracking-tight text-slate-800 md:text-[28px] dark:text-white">
                                {isTwoFactorPage ? "Two-Factor Authentication" : "Account Settings"}
                            </h1>
                        ...
                        <div className="shrink-0">
                            {isTwoFactorPage ? (
                                <Button asChild variant="outline" className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                    <Link href={edit()}>
                                ...
                                <Button asChild className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow-xs">
                                    <Link href={show()}>
