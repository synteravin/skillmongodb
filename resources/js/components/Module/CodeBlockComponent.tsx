...
function highlightJsxLine(line: string): React.ReactNode[] {
    ...
    while ((match = tokenRegex.exec(line)) !== null) {
        ...
        if (comment) {
            parts.push(
                <span key={match.index} className="italic text-slate-400 dark:text-slate-500">
                    {comment}
                </span>
            );
            prevKeyword = false;
            ...
            parts.push(
                <span key={match.index} className="font-medium text-amber-600 dark:text-[#fbbf24]">
                    {stringVal}
                </span>
            );
            prevKeyword = false;
            ...
            parts.push(
                <span key={match.index}>
                    <span className="text-slate-400 dark:text-slate-400">{isClosing ? '</' : '<'}</span>
                    <span
                        className={
                            isComponent
                                ? 'font-semibold text-indigo-600 dark:text-[#60a5fa]'
                                : 'font-semibold text-cyan-600 dark:text-[#38bdf8]'
                        }
                    >
                        {tagName}
                    </span>
            ...
            parts.push(
                <span key={match.index}>
                    <span className="text-purple-600 dark:text-[#c084fc]">{attrName}</span>
            ...
            parts.push(
                <span key={match.index} className="text-slate-400 dark:text-slate-400">
                    {tagClose}
                </span>
            );
            prevKeyword = false;
            ...
            parts.push(
                <span key={match.index} className="font-semibold text-blue-600 dark:text-[#38bdf8]">
                    {keyword}
                </span>
            );
            ...
            parts.push(
                <span key={match.index} className="font-semibold text-indigo-600 dark:text-[#60a5fa]">
                    {capitalizedName}
                </span>
            );
            prevKeyword = false;
            ...
            parts.push(
                <span key={match.index} className="text-amber-600 dark:text-[#fbbf24]">
                    {numberVal}
                </span>
            );
            prevKeyword = false;
            ...
            parts.push(
                <span key={match.index} className="text-slate-500 dark:text-slate-400">
                    {operator}
                </span>
            );
            prevKeyword = false;
            ...
            if (prevKeyword) {
                ...
                parts.push(
                    <span key={match.index} className="text-slate-800 dark:text-slate-100">
                        {identifier}
                    </span>
                );
            }
            ...
            parts.push(
                <span key={match.index} className="text-slate-700 dark:text-slate-300">
                    {full}
                </span>
            );
            prevKeyword = false;
        ...
        if (comment) {
            ...
            parts.push(
                <span key={match.index}>
                    <span className="text-purple-600 dark:text-[#c084fc]">{attrName}</span>
                    <span className="text-slate-400 dark:text-slate-400">=</span>
                    <span className="font-medium text-amber-600 dark:text-[#fbbf24]">{attrVal}</span>
                </span>
            );
        } else if (tagClose) {
            parts.push(
...
function highlightTerminalLine(line: string): React.ReactNode[] {
    if (
        ...
        return [
            <span key="prompt" className="mr-2 font-bold text-rose-500 dark:text-rose-400">
                {prompt}
            </span>,
            <span key="cmd" className="font-semibold text-blue-700 dark:text-sky-300">
                {rest}
            </span>,
        ];
    }
    return [<span key="out" className="text-slate-700 dark:text-slate-300">{line}</span>];
}
...
export default function CodeBlockComponent({
    ...
    return (
        <div
            className={`my-4 overflow-hidden rounded-xl border border-slate-200 bg-slate-50 text-slate-900 shadow-sm transition-colors dark:border-slate-800/90 dark:bg-[#040711] dark:text-slate-100 ${className}`}
        >
            {/* TOP BAR / IDE HEADER */}
            <div className="flex flex-wrap items-center justify-between border-b border-slate-200 bg-slate-100/90 px-4 py-2.5 sm:px-5 dark:border-slate-800/90 dark:bg-[#0d121f]">
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    {title && (
                        <span className="ml-2 font-mono text-xs font-semibold text-slate-600 dark:text-slate-300">
                            {title}
                        </span>
                    )}
                </div>
                ...
                <div className="flex items-center gap-3">
                    <span className="font-mono text-[10px] font-bold tracking-wider uppercase text-slate-500 sm:text-xs dark:text-slate-400">
                        {displayLabel}
                    </span>
                    ...
                    <button
                        type="button"
                        onClick={handleCopy}
                        className="flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1 text-[11px] font-medium text-slate-700 transition-all hover:border-slate-400 hover:bg-slate-100 hover:text-slate-900 active:scale-95 dark:border-slate-700/80 dark:bg-slate-800/60 dark:text-slate-300 dark:hover:border-slate-600 dark:hover:bg-slate-700 dark:hover:text-white"
                        title="Copy Code"
                    >
                        {copied ? (
                            <>
            ...
            <div className="overflow-x-auto p-4 font-mono text-xs leading-relaxed text-slate-800 sm:p-5 sm:text-sm dark:text-slate-100">
                <table className="w-full border-collapse">
                    <tbody>
                        {lines.map((line, idx) => (
                            <tr key={idx} className="hover:bg-slate-200/50 dark:hover:bg-slate-800/30">
                                {showLineNumbers && (
                                    <td className="w-10 pr-4 text-right font-mono text-xs text-slate-400 select-none align-top dark:text-slate-600">
                                        {idx + 1}.
                                    </td>
                                )}
