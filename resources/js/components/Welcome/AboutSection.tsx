...
function Card1Background({ width, height, isMd }: CardBackgroundProps) {
    if (!isMd || width === 0 || height === 0) return null;
...
export default function AboutSection() {
    ...
    const card1Class = `relative z-10 p-6 sm:p-8 transition-all duration-300 ${
        isMd && card1Measured
            ? 'border-transparent bg-transparent shadow-none'
            : 'rounded-[24px] sm:rounded-[28px] border border-cyan-400/40 bg-[#1D215D] shadow-[0_12px_30px_rgba(0,0,0,0.4)] md:shadow-[0_15px_35px_rgba(0,0,0,0.45)] lg:shadow-[0_8px_25px_rgba(0,0,0,0.35)]'
    }`;

    const card2Class = `relative z-10 p-6 sm:p-8 transition-all duration-300 ${
        isMd && card2Measured
            ? 'border-transparent bg-transparent shadow-none'
            : 'rounded-[24px] sm:rounded-[28px] border border-cyan-400/40 bg-[#1D215D] shadow-[0_12px_30px_rgba(0,0,0,0.4)] md:shadow-[0_15px_35px_rgba(0,0,0,0.45)] lg:shadow-[0_8px_25px_rgba(0,0,0,0.35)]'
    }`;
    ...
    return (
        <section id="about" className="relative py-8 sm:py-16 md:py-24 2xl:py-32">
            {/* Ambient Tech Glows */}
            <div className="pointer-events-none absolute top-16 -right-14 z-0 hidden sm:block">
                <div className="h-[220px] w-[220px] rounded-full bg-blue-800 blur-[70px]" />
            </div>
            <div className="pointer-events-none absolute bottom-8 -left-12 z-0">
                <div className="h-[200px] w-[200px] rounded-full bg-blue-800 blur-[70px]" />
            </div>

            <div className="relative z-10 container mx-auto px-4 sm:px-6">
                <div className="mx-auto max-w-4xl 2xl:max-w-6xl">
                    {/* 🔥 CARD UTAMA */}
                    <div className="relative rounded-2xl bg-[#13174D] px-4 py-8 shadow-[0_10px_30px_rgba(0,0,0,0.35)] sm:rounded-3xl sm:px-8 sm:py-14 md:rounded-tl-[12px] md:rounded-tr-[32px] md:rounded-br-[55px] md:rounded-bl-[55px] md:px-12 md:py-20 md:shadow-none">
                        ...
                        <div className="pointer-events-none absolute inset-0 z-[40]">
                            {/* 🔥 PNG 1 — PHONE (KIRI ATAS / OUTER EDGE MOBILE) */}
                            <div className="absolute -top-3 left-3 sm:-top-4 sm:left-6 md:top-[20px] md:left-[180px] lg:left-[280px]">
                                <img
                                    src="/images/phone.webp"
                                    alt="phone"
                                    className="w-12 drop-shadow-2xl sm:w-16 md:w-24 lg:w-18"
                                />
                            </div>

                            {/* 🔥 PNG 2 — MONITOR (LEFT GAP / OUTER EDGE MOBILE) */}
                            <div className="absolute top-[48%] -left-3 -translate-y-1/2 sm:top-[47%] sm:-left-4 md:top-auto md:bottom-[1px] md:left-[30px] md:translate-y-0 lg:left-[60px]">
                                <img
                                    src="/images/monitor.webp"
                                    alt="monitor"
                                    className="w-14 drop-shadow-2xl sm:w-18 md:w-28 lg:w-24"
                                />
                            </div>

                            {/* 🔥 PNG 3 — GAMEPAD MIRING (KANAN BAWAH / OUTER EDGE MOBILE) */}
                            <div className="absolute -bottom-3 -right-2 rotate-[5deg] sm:-bottom-4 sm:right-2 md:bottom-[40px] md:right-[5px] lg:bottom-[60px]">
                                <img
                                    src="/images/gamepad.webp"
                                    alt="gamepad"
                                    className="w-14 drop-shadow-2xl sm:w-18 md:w-28 lg:w-32"
                                />
                            </div>
                        </div>

                        {/* 🔥 GRID WRAPPER */}
                        <div className="grid gap-6 sm:gap-8 md:grid-cols-2 md:gap-10 lg:gap-16 xl:gap-20">
                            {/* 🔥 MOBILE HEADER */}
                            <div className="col-span-full mb-1 flex justify-center sm:mb-4 md:hidden">
                                <div className="rounded-full border border-cyan-300/60 bg-white/5 px-5 py-2 backdrop-blur-md">
                                    <span className="font-['Orbitron'] text-sm tracking-[3px] text-cyan-200 sm:text-base">
                                        About Section
                                    </span>
                                </div>
                            </div>
                            ...
                            {/* ================= CARD 1 ================= */}
                            <div className="relative md:translate-x-4 lg:translate-x-0">
                                <div ref={card1Ref} className={card1Class}>
                                    ...
                                    <div className="absolute -top-2.5 left-6 h-1.5 w-14 rounded-full bg-cyan-400 md:hidden" />
                            ...
                            <div className="relative md:-translate-x-4 lg:translate-x-0">
                                <div ref={card2Ref} className={card2Class}>
                                    ...
                                    {/* Zigzag Accent (mobile only) */}
                                    <div className="absolute -top-2.5 right-6 h-1.5 w-14 rounded-full bg-pink-400 md:hidden" />
                        ...
                        {/* 🔥 LEKUKAN BAWAH */}
                        <div className="absolute bottom-0 left-1/2 hidden h-[50px] w-[180px] -translate-x-1/2 rounded-t-[24px] bg-[#f8fafc] sm:h-[60px] sm:w-[220px] md:block md:h-[70px] md:w-[260px] md:rounded-t-[32px] dark:bg-[#020202]" />
                    </div>
                </div>
            </div>
        </section>
