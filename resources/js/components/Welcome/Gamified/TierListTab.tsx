...
const authenticPlayers: Player[] = [
    {
        ...
        rank: {
            name: 'Hydra',
            image: '/images/ranks/ztTU3GkHJZTJDrfDddmoKvlob5f8fWSIPhWap69z.png',
            star: 3,
        },
    },
    {
        ...
        rank: {
            name: 'Hydra',
            image: '/images/ranks/XWDGHrIh6LdBB6pVlSmDsY1fcR87voqHcEW3vdUn.png',
            star: 2,
        },
    },
    {
        id: 'u3',
        name: 'Dimas Setiawan (Anda)',
        avatar: '/images/default-avatar.svg',
        total_score: 2150,
        position: 3,
        rank: {
            name: 'Hatchling',
            image: '/images/ranks/6WSSbYgyA1mDS3Pemc80j80q35Lk3ad0bP6wP5HZ.png',
            star: 3,
        },
    },
    {
        ...
        rank: {
            name: 'Hatchling',
            image: '/images/ranks/cWstY7eDoyOGXwQIGY07Edk5JeIGiwe54enhSkJP.png',
            star: 2,
        },
    },
    {
        ...
        rank: {
            name: 'Hatchling',
            image: '/images/ranks/IRs7DcnwkDPlB6JDbwoB5JNaVIFmOsfMVRJwGGZ0.png',
            star: 1,
        },
    },
];
...
export default function TierListTab() {
    // Siswa aktif saat ini (akun Anda)
    const currentUserId = 'u3';
    const [selectedPlayerId, setSelectedPlayerId] = useState<string>('u3');

    const activeUser =
        authenticPlayers.find((p) => p.id === selectedPlayerId) ||
        authenticPlayers[0];
    ...
    return (
        <div className="relative flex w-full flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-[#fdfcfc] p-3 text-slate-800 shadow-2xl transition-colors duration-500 sm:p-6 md:p-8 dark:border-blue-500/20 dark:bg-[#020202] dark:text-white text-left">
            ...
            <div className="relative z-10 mx-auto flex min-h-0 w-full flex-1 flex-col">
                {/* ═══════════════════════════════════════════════════════════════
                    HEADER (1:1 Exact Replica from Student/Leaderboard.tsx)
                   ═══════════════════════════════════════════════════════════════ */}
                <div className="-mt-1 mb-4 flex shrink-0 items-center gap-3 sm:-mt-3 sm:mb-6 sm:gap-6 md:mb-8">
                    {/* Tombol Back Futuristik Poligon Asli */}
                ...
                {/* ═══════════════════════════════════════════════════════════════
                    MAIN CONTENT AREA (1:1 Exact Split Grid from Student/Leaderboard.tsx)
                   ═══════════════════════════════════════════════════════════════ */}
                <div className="grid h-full min-h-0 flex-1 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-12 lg:gap-8">
                    {/* ── LEFT: RANK VISUAL (Level Rank Emblem Asli) ── */}
                    {activeUser && (
                        <div className="flex shrink-0 flex-col items-center justify-center p-2 sm:p-4 lg:col-span-4">
                            <div className="flex w-full flex-col items-center justify-center gap-1.5 text-center">
                                ...
                                {/* Authentic Level Rank Emblem with Sharp Glowing Neon Aura */}
                                <div className="relative flex shrink-0 items-center justify-center p-1">
                                    <img
                                        ...
                                        alt={activeUser.rank?.name ?? 'Level Rank'}
                                        className="relative z-10 h-24 w-24 object-contain transition-all duration-300 sm:h-36 sm:w-36 md:h-48 md:w-48 lg:h-56 lg:w-56 xl:h-60 xl:w-60"
                                        style={{
                    ...
                    {/* ── RIGHT: LIST CONTAINER (1:1 Exact Glass Container) ── */}
                    <div className="flex h-full min-h-0 flex-col lg:col-span-8">
                        <div className="relative flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-3xl border border-blue-200 bg-white/80 p-4 shadow-xl backdrop-blur-md sm:p-6 dark:border-blue-500/30 dark:bg-black/60 dark:shadow-2xl">
                            ...
                            {/* SCROLLABLE PLAYER LIST (Interactive hover to inspect on left) */}
                            <div className="custom-scrollbar mt-3 -mr-2 min-h-0 flex-1 space-y-3 overflow-y-auto pr-2">
                                {authenticPlayers.map((player, index) => {
                                    const isCurrentUser =
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`group grid cursor-pointer grid-cols-12 items-center rounded-2xl border p-2.5 transition-all duration-300 sm:p-3 ${
                                                isCurrentUser
                                        ...
                                        >
                                            ...
                                            {/* SCORE (Pure number without 'PTS' suffix, exact 1:1) */}
            ...
            {/* Scoped CSS for custom scrollbar (1:1 from Student/Leaderboard.tsx) */}
            <style>{`
