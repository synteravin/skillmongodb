...
export default function StudentCareerBranch({
    ...
    return (
        <div className="text-sans relative flex w-full flex-col items-center px-8 sm:px-4">
                    ...
                    >
                        ...
                        {group.mentor ? (
                            ...
                            >
                                ...
                                group.mentor.avatar !== 'null' ? (
                                    <img
                                        ...
                                        className="h-10 w-10 flex-shrink-0 rounded-full border border-gray-400 object-cover shadow-sm transition-all group-hover/mentor:scale-105 group-hover/mentor:border-indigo-400"
                                        alt="mentor"
                                    />
                                ) : (
                                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-blue-500 bg-gradient-to-br from-blue-500 to-indigo-600 transition-all group-hover/mentor:scale-105 group-hover/mentor:border-indigo-400">
                                ...
                                <div className="flex flex-col truncate">
                                    <span
                                        className={`truncate text-[10px] leading-none font-bold transition-colors ${
                                            !basicCompleted || isOtherChosen
                                                ? 'text-blue-400 dark:text-gray-500'
                                                : 'text-gray-900 group-hover/mentor:text-indigo-500 dark:text-[#F0F0F0] dark:group-hover/mentor:text-[#00d4ff]'
                                        }`}
                                    >
                                        {group.mentor.name}
                                    </span>
                                    <span
                                        className={`mt-0.5 truncate text-[8px] ${
                                            !basicCompleted || isOtherChosen
                                                ...
                                                : 'dark:text-gray-450 text-gray-400'
                                        }`}
                                    >
                            ...
                            <div className="flex max-w-[60%] items-center gap-2">
                                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-blue-200 bg-slate-200 dark:border-blue-900 dark:bg-slate-800">
                        ...
                        {isCompleted ? (
                            <button
                                disabled
                                className="flex-shrink-0 rounded-xl border-2 border-emerald-500 bg-emerald-500/10 px-4 py-1 font-['Orbitron'] text-[12px] font-bold tracking-widest text-emerald-400 uppercase shadow-[0_0_8px_1px_rgba(52,211,153,0.3)]"
                            >
                                Finish
                            </button>
                        ) : (
                            <button
                                ...
                                className={`flex-shrink-0 rounded-xl border-2 px-4 py-1 font-['Orbitron'] text-[12px] font-bold tracking-widest uppercase transition-all duration-300 ${
                                    !basicCompleted || isOtherChosen
                                        ...
                                        : `border-sky-300 bg-sky-50 text-sky-700 shadow-[0_3px_10px_rgba(56,189,248,0.22),inset_0_0_5px_rgba(255,255,255,0.7)] hover:border-sky-400 hover:bg-sky-100 hover:text-sky-800 hover:shadow-[0_5px_12px_rgba(56,189,248,0.28),inset_0_0_6px_rgba(255,255,255,0.8)] dark:border-[#3B28F6] dark:bg-[#05080f] dark:text-white dark:shadow-[0_3px_10px_rgba(59,40,246,0.35),inset_0_0_5px_rgba(59,40,246,0.12)] dark:hover:border-[#5a47ff] dark:hover:bg-[#080d1a] dark:hover:shadow-[0_5px_12px_rgba(59,40,246,0.45),inset_0_0_6px_rgba(59,40,246,0.16)]`
                                }`}
                            >
                                Start
                            </button>
            ...
            <div
                className={`h-8 w-[2px] bg-blue-500 dark:bg-gray-500 ${isOtherChosen ? 'opacity-30' : ''}`}
            />

            {/* ── PATHS LIST ── */}
            <div
                className={`flex w-full flex-col items-center ${!basicCompleted || isOtherChosen ? 'opacity-60' : ''}`}
                /* ↑ grayscale dihapus, cukup opacity ringan */
            >
                <div
                    className={`h-8 w-[2px] bg-blue-200 dark:bg-gray-500 ${isOtherChosen ? 'opacity-30' : ''}`}
                />

                <div
                    className={`flex w-full flex-col items-center ${
                        !basicCompleted ||
                        (!hasChosenPath && !isCompleted) ||
                        isOtherChosen
                            ...
                            locked={
                                ...
                                (!basicCompleted ||
                                    ...
                                    !progress.completed_paths?.includes(
                                        String(
                                            group.paths[group.paths.length - 1]
                                                ._id,
                                        ),
                                    ))
                            }
